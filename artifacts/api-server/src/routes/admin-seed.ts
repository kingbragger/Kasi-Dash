...
const SEED_PRODUCTS = [
  ...
  },
  {
    ...
    description: "Apple iPhone 11 unlocked, all storage and colour options. A13 Bionic, 4K video, Face ID. Very good refurbished condition.",
    price: "2802.29",
  ...
  },
  {
    ...
    description: "Dell Latitude 3390 2-in-1. Intel Core i5-8350U, 8GB RAM, 128GB SSD, 13.3\" FHD touchscreen, Windows 11 Pro.",
    price: "3019.70",
...
router.post("/admin/seed-products", requireAuth, requireAdmin, async (req, res) => {
  try {
    let inserted = 0;
    for (const product of SEED_PRODUCTS) {
      const result = await db.insert(productsTable).values(product).onConflictDoNothing().returning({ id: productsTable.id });
      if (result.length > 0) inserted++;
    }
    res.json({ inserted, total: SEED_PRODUCTS.length, message: `Seeded ${inserted} new products (${SEED_PRODUCTS.length - inserted} already existed).` });
  } catch (err) {
    req.log.error({ err }, "Failed to seed products");
